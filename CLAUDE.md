...
This repo is also the **single place where blog topics are proposed and
scheduled**. Ideas do not go on the site. Only dated posts do.
...
- **Intake.** Turn topic suggestions — from Slack, issues, or conversation —
  ...
  `status: idea`. You never schedule your own intake.
...
| `scheduled` | has an author and `publish_date` | no | yes | yes, as "Coming soon" |
| `published` | live | no | yes | yes, as a post |
| `declined` | won't be published | yes (separate section) | no | no |
...
an `idea` with a `target_date` is still an idea. It exists so the dashboard and `CALENDAR.md` can show intent instead of
an empty calendar. Only `publish_date` schedules a post.
...
- **A draft is not a date.** A finished, polished draft in Box changes
  nothing about a topic's `status`; the sequence is still idea → scheduled →
  published.
...
There is a shared HTML dashboard — calendar on top, topic status below —
...
added, removed, or renamed; any `status`, `publish_date`,
...
- Stat tiles (idea / scheduled / published / declined) and the topic
...
- Any prose that counts things ("4 of 6 have no author") — these go stale silently.
...
happens, say so — in the channel or the PR — and name the ideas that could take the
next open slot. A thin schedule is the failure mode to watch for, and it
